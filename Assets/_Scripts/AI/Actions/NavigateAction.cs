...
namespace Game.Scripts.AI.Actions
{
	...
	[RequireComponent(typeof(NavigatingState))]
	public class NavigateAction : ReGoapAction<string, object>
	{
		...
		protected NavigatingState smsGoto;

		public const string KEY_ISATPOSITION = "isAtPosition:";

		protected override void Awake()
		{
			base.Awake();
			SetDefaultEffects();
			smsGoto = GetComponent<NavigatingState>();
		}

		protected virtual void SetDefaultEffects()
		{
			effects.Set(KEY_ISATPOSITION, default(Vector3));
		}
		...
		{
			return agent.GetMemory().GetWorldState().Get(KEY_ISATPOSITION) as Vector3?;
		}
		...
		public override void Run(IReGoapAction<string, object> previous, IReGoapAction<string, object> next, IReGoapActionSettings<string, object> settings, ReGoapState<string, object> goalState, Action<IReGoapAction<string, object>> done, Action<IReGoapAction<string, object>> fail)
		{
			...
			var localSettings = (GenericNavigateSettings)settings;
		...
		public override ReGoapState<string, object> GetEffects(ReGoapState<string, object> goalState, IReGoapAction<string, object> next = null)
		{
			...
			{
				effects.Set(KEY_ISATPOSITION, goalWantedPosition);
			}
			else
			{
				SetDefaultEffects();
		...
		Vector3? GetWantedPositionFromState(ReGoapState<string, object> state)
		{
			...
			if (state != null)
			{
				result = state.Get(KEY_ISATPOSITION) as Vector3?;
			}
			return result;
		}
		...
		{
			settings = new GenericNavigateSettings
			{
		...
		{
			Debug.Log("Cost: " + (base.GetCost(goalState, next) + Cost));
	...
	public struct GenericNavigateSettings : IReGoapActionSettings<string, object>
	{
